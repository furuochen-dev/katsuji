...
describe('一行上的步骤和跳过', function () {
  it('段首行也做第 3 步；不是末行就做 5', function () {
    assert.deepEqual(lineStepPlan(0, 3), { step3: true, step4: true, step5: true });
  });
  ...
  it('只有一行：做 3、4，跳过 5', function () {
    assert.deepEqual(lineStepPlan(0, 1), { step3: true, step4: true, step5: false });
  });

  it('两行：第一行 3+4+5，第二行 3+4', function () {
    assert.deepEqual(lineStepPlan(0, 2), { step3: true, step4: true, step5: true });
